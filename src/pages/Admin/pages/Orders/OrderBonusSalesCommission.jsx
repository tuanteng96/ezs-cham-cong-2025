...
function OrderBonusSalesCommission({ f7route, f7router }) {
  ...
  const { data, isLoading, refetch } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { doanh_so, hoa_hong, oiItems } = newData;
      let newObj =
      ...
      newData["Fields"] = newObj
        ? newObj.filter(
            (x) =>
              (x?.Hoa_Hong && x?.Hoa_Hong.length > 0) ||
              (x?.Doanh_So && x?.Doanh_So.length > 0)
          )
        : [];
      return newData || null;
    },
  ...
  let isFields =
    (data && data?.Fields?.some((x) => x.Doanh_So && x.Doanh_So.length > 0)) ||
    (data && data?.Fields?.some((x) => x.Hoa_Hong && x.Hoa_Hong.length > 0));
    ...
    >
      <Navbar innerClass="!px-0 text-white" outline={false}>
        ...
        <NavRight className="h-full">
          <PickerAddSalesCommission Order={data} ref={pickerRef} f7router={f7router}>
            {({ open }) => (
              <Link
                onClick={open}
                noLinkClass
              ...
              {fields &&
                fields.map((item, index) => (
                  <div
                    ...
                    key={item.id}
                  >
                    <div className="flex items-center gap-3 px-4 py-2.5 border-b bg-warning-light rounded-t">
                      ...
                      {ConditionsHelpers.isDeleteProductSalesSommission(
                        ...
                        <div
                          className="flex items-center justify-end w-10 h-8 text-danger"
                          onClick={() => {
                            remove(index);
                          }}
                        >
