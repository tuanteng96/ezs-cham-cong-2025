...
function PosLocksCalendar(props) {
  ...
  const { data, isLoading, refetch } = useQuery({
    queryKey: ["ConfigLocks"],
    queryFn: async () => {
      let { data } = await ConfigsAPI.getValue("giocam");
      let result = [];
  ...
  const onSubmit = (values) => {
    ...
    if (index > -1) {
      newValues[index].ListDisable = values.ListDisable;
    }

    newValues = newValues.map((item) => ({
      ...item,
      ListDisable: item.ListDisable.map((x) => ({
        ...x,
        Date: moment(x.Date).format("DD/MM/YYYY"),
        TimeClose: x.TimeClose.map((o) => ({
          ...o,
          Start: moment(o.Start).format("HH:mm"),
          End: moment(o.End).format("HH:mm"),
        })),
      })),
    }));

    updateMutation.mutate(
      { data: newValues, name: "giocam" },
      {
        onSuccess: () => {
  ...
  return (
    ...
    >
      <Navbar innerClass="!px-0 text-white" outline={false}>
        ...
        <NavTitle>Cài đặt khoá lịch</NavTitle>
            ...
            <div className="p-4 overflow-auto grow">
              {fields &&
                ...
                fields.map((field, index) => (
                  ...
                  >
                    ...
                    <FieldsTimes name={`ListDisable[${index}].TimeClose`} />
                  </div>
                ))}
              ...
              {(!fields || fields.length === 0) && (
                <NoFound
                  ...
                  Desc="Vui lòng chọn (+) để thêm mới khoá lịch."
                />
              )}
            </div>
          )}
...
export default PosLocksCalendar;
