...
function PosClientManage({ f7route, f7router }) {
        ...
        if (x.Os && x.Os.length > 0) {
          for (let OsItem of x.Os) {
            if (OsItem.OrderItem.OrderID === x.OrderID) {
              let newOsItem = { ...OsItem };
              newOsItem.Services = OsItem?.Services?.map((osi) => {
                let newOsi = { ...osi };
                if (!newOsi?.Staffs || newOsi?.Staffs.length === 0) {
                  if (newOsi.Staffs2 && newOsi.Staffs2.length > 0) {
                    newOsi.Staffs = [...newOsi.Staffs2];
                  }
                }
        ...
        if (
          ...
          newRs.OrderItems = newRs.OrderItems.map((OrderItem) => {
            ...
            if (index > -1) {
              ...
              newOrderItem.AdditionalFees = newRs.OrderItems.map((x) => {
                let newObj = { ...x };
                if (x.IsAddFee) {
                  let indexFee =
                    IDFees && IDFees.length > 0
                      ? IDFees.findIndex((k) => Number(k) === x.ProdType)
                      : -1;
                  if (indexFee > -1) {
                    newObj["ProdTypeTitle"] = x?.TypeTitle || "Chưa xác định";
                  }
                }
                return newObj;
              }).filter((x) => x?.ProdTypeTitle);
            }
            return newOrderItem;
          });
  ...
  const OrderOverview = useQuery({
    queryKey: ["OrderOverviewID", { ID: Order?.data?.Order?.ID }],
    queryFn: async () => {
      let { data } = await AdminAPI.clientsOrderOverviewId({
        OrderID: Order?.data?.Order?.ID,
        Token: Auth?.token,
      });
      return data?.data || null;
    },
    enabled: Boolean(Order?.data?.Order?.ID > 0 && isPopover),
  });
  ...
  const wrapTypeFees = (Items) => {
    let map = new Map();

    for (let item of Items) {
      if (!map.has(item.ProdType)) {
        map.set(item.ProdType, {
          ID: item.ProdType,
          Title: item.ProdTypeTitle,
          Items: [item],
        });
      } else {
        map.get(item.ProdType).Items.push(item);
      }
    }
  ...
  const onNailDelete = (item) => {
    f7.dialog.confirm("Bạn có chắc chắn muốn xoá mặt hàng này ?", () => {
      ...
      bodyFormData.append(
        "deleteIds",
        [item?.ID, ...(item?.AdditionalFees || []).map((x) => x.ID)].toString()
      );
      ...
      <Navbar innerClass="!px-0 text-white" outline={false}>
        <div className="flex w-full h-full navbar-custom">
          <Link
            ...
            onClick={() => {
              if (f7router?.previousRoute?.path === "/admin/pos/clients/add/") {
        ...
        {!Client?.isLoading &&
          ...
          Client?.data?.CheckIn?.StockID !== CrStocks?.ID && (
            <div className="px-3 pt-3">
              <div className="flex justify-center gap-1 px-4 py-3.5 border-b text-danger bg-white rounded-lg mb-3.5 last:mb-0">
                <ExclamationTriangleIcon className="w-5" />
                Đang Check In tại
                <span>
                  {Auth?.Info?.StocksAll?.find(
                    (x) => x.ID === Client?.data?.CheckIn?.StockID
                  )?.Title || "Chưa xác định"}
                </span>
              </div>
            </div>
                            ...
                            >
                              <div className="flex items-center gap-3 p-4">
                                  ...
                                  Options={[
                                    {
                                      Title: "Thực hiện xoá",
                                      className:
                                        "flex items-center justify-center h-[54px] border-b last:border-0 text-[15px] cursor-pointer text-danger font-medium",
                                      onClick: (e) => {
                                        onNailDelete(item);
                                      },
                                    },
                                ...
                                >
                                  {({ open }) => (
                                    <div
                                      className="flex items-center justify-end w-10"
                                      onClick={open}
                                    >
                              ...
                              {item.AdditionalFees &&
                                item.AdditionalFees.length > 0 && (
                                  <div className="px-4 pb-4">
                                    {wrapTypeFees(item.AdditionalFees).map(
                                      (sub, k) => (
                                        <div
                                          className="relative mb-1.5 last:mb-0"
                                          key={k}
                                        >
                                          <div className="py-2 truncate absolute left-0 top-1/2 -translate-y-1/2 [writing-mode:vertical-rl] rotate-180 text-sm whitespace-nowrap text-center h-full bg-warning-light rounded">
                                            {sub.Title}
                                          </div>
          ...
          <div className="px-4 py-3 border-t">
            ...
            <div className="flex gap-2">
              ...
              <Popover
                ...
                onPopoverOpen={() => setIsPopover(false)}
                onPopoverClosed={() => {
                  setIsPopover(false);
                ...
                <div className="flex flex-col py-1">
                  ...
                  Order?.data?.OrderItems?.length > 0 ? (
                    <>
                      <Link
                        ...
                        noLinkClass
                        href={`/admin/pos/orders/view/${Order?.data?.Order?.ID}/bonus-sales-commission/?isBack=1`}
                  ...
                  Order?.data?.OrderItems?.length > 0 ? (
                    <>
                      ...
                      <PickerAff
                        data={OrderOverview?.data?.AffM}
                        Order={Order?.data?.Order}
                        OrderItems={Order?.data?.OrderItems}
                        MoreInfo={OrderOverview?.data}
                      >
                        {({ open }) => (
                          <Link
                            popoverClose
                          ...
                          >
                            ...
                            {!OrderOverview?.isLoading && (
                              <>
                                {OrderOverview?.data?.AffM?.length > 0 && (
                                  <div className="px-1.5 font-semibold text-xs text-white rounded bg-danger font-lato flex items-center">
                                    {OrderOverview?.data?.AffM?.length}
                                  </div>
                                )}
                              </>
                            )}
                      ...
                      <PickerAccumulate
                        data={OrderOverview?.data?.TAKE_MM}
                        Order={Order?.data?.Order}
                      >
                        {({ open }) => (
                          <Link
                          ...
                          >
                            ...
                            {!OrderOverview?.isLoading && (
                              <>
                                {OrderOverview?.data?.TAKE_MM?.length > 0 && (
                                  <div className="px-1.5 font-semibold text-xs text-white rounded bg-danger font-lato flex items-center">
                                    {OrderOverview?.data?.TAKE_MM?.length}
                                  </div>
                                )}
                              </>
                            )}
                  ...
                  {Order?.data?.mck?.ID ? (
                    <PickerInvoiceOrder Order={Order?.data}>
                      {({ open }) => (
                        <Link
                          popoverClose
                          className="flex justify-between p-3 font-medium border-b last:border-0"
                  ...
                  <Button
                    onClick={() => {
                      if (Order?.data?.Order?.ToPay > 0) {
                        ...
                        f7.views.main.router.navigate(
                          `/admin/printers/order/${Order?.data?.Order?.ID}/`
                        );
                      }
                    }}
                    type="button"
